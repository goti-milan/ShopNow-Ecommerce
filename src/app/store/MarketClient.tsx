    ...
    <div className="overflow-hidden rounded-xl border border-border bg-background">
      ...
      <div className="space-y-3 p-3">
        ...
        <div className="text-sm text-foreground">
          ...
          <span className="ml-1 text-muted-foreground">({store.reviews})</span>
        </div>

        <p className="text-sm text-foreground">
          <span className={`font-semibold ${store.isOpen ? "text-primary" : "text-primary-dark"}`}>
        ...
        <div className="flex items-center justify-between border-t border-border pt-3">
          <span className="text-sm font-semibold text-muted-foreground">{store.distance}</span>
          <Link href={`/store/${store.id}`}>
            <Button size="sm" className="rounded-lg bg-primary hover:bg-primary-dark">
              Visit Store
            </Button>
          </Link>
        </div>
      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex items-start justify-between gap-2">
            <div>
              <h3 className="truncate text-2xl font-bold text-foreground">{store.name}</h3>
              <p className="text-sm text-muted-foreground">{store.category}</p>
            </div>
            <span className="text-xl font-semibold text-foreground">{store.distance}</span>
          </div>

          <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground">
            <span className="inline-flex items-center gap-1 font-semibold text-primary">
              <Star className="h-4 w-4 fill-primary" /> {store.rating}
            </span>
            ...
            <div className="space-y-4">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                ...
                <div>
                  <button
                    type="button"
                    className="mb-2 text-sm font-semibold text-primary-dark hover:text-primary-dark"
                    onClick={() => router.push("/store")}
                  >
                    ← Back to store home
                  </button>
                  <h1 className="text-4xl font-bold text-foreground">Nearby Stores</h1>
                  <p className="mt-1 text-muted-foreground">Find stores directly from map and distance filters</p>
                </div>
            ...
            <aside className="space-y-4">
              <section className="rounded-2xl border border-border bg-background p-4 shadow-sm">
                <div className="mb-3 flex items-center justify-between">
                  <h2 className="text-xl font-bold text-foreground">Filter Stores</h2>
                  <span className="text-xs font-semibold text-muted-foreground">{filteredStores.length} results</span>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      className={`inline-flex h-9 shrink-0 items-center rounded-full px-3 text-xs font-semibold ${openNow ? "bg-accent text-primary-dark" : "bg-muted text-muted-foreground"
                        }`}
                    >
    ...
    <div className="min-h-screen bg-muted pb-10">
      <div className="container mx-auto space-y-5 px-4 py-6">
        <section className="rounded-2xl border border-border bg-background p-4 shadow-sm">
          <div className=" flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h1 className="text-4xl font-bold text-foreground">Store Listings</h1>
              <p className="mt-1 text-lg text-muted-foreground">Find best stores near you</p>
            </div>
            <div className="relative w-full lg:w-[360px]">
              <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
              <Input placeholder="Search stores, products..." className="h-11 bg-background pl-10" />
            </div>
          </div>
        </section>
          ...
          <div className="grid gap-3 md:grid-cols-4 lg:grid-cols-7">
            {categories.map((item) => (
              <button
                key={item.label}
                type="button"
                className="flex items-center gap-2 rounded-xl border border-border bg-muted px-3 py-3 text-left text-sm font-semibold text-foreground transition hover:bg-background"
              >
        ...
        <div className="flex flex-col gap-6 lg:flex-row">
          <aside className="space-y-4 lg:w-72">
            <section className="rounded-2xl border border-border bg-background p-4 shadow-sm">
              <h2 className="mb-3 text-xl font-bold text-foreground">Filter & Search</h2>
              <div className="space-y-3">
                ...
                <button
                  type="button"
                  className="flex w-full items-center justify-between rounded-lg border border-border px-3 py-2 text-left text-sm text-muted-foreground"
                >
                ...
                <Link href="/store?view=all">
                  <Button className="w-full rounded-lg bg-primary hover:bg-primary-dark">Show all stores</Button>
                </Link>
              </div>
            </section>
            <section className="rounded-2xl border border-border bg-background p-4 shadow-sm">
              <h2 className="mb-3 text-xl font-bold text-foreground">Top Rated Stores</h2>
              <div className="space-y-3">
                {topRated.map((item) => (
                  <Link
                    key={item.id}
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="inline-flex items-center gap-1 text-sm font-semibold text-primary">
                        <Star className="h-4 w-4 fill-primary" /> {item.rating}
                      </p>
            ...
            <section className="overflow-hidden rounded-2xl border border-border bg-background shadow-sm">
              <div className="relative h-56">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/65 via-black/15 to-transparent p-4">
                  <div className="absolute bottom-4 left-4 right-4 rounded-xl bg-background/95 p-3">
                    <p className="text-xs font-semibold uppercase tracking-wide text-primary-dark">Offers & deals</p>
                    <h3 className="mt-1 text-2xl font-bold text-foreground">Flat 30% OFF</h3>
                    <p className="text-sm text-muted-foreground">On all stores</p>
                    <Link href="/store?view=all">
                      <Button size="sm" className="mt-3 rounded-lg bg-primary hover:bg-primary-dark">
                        Grab now
                      </Button>
                    </Link>
                  </div>
          ...
          <div className="flex-1 space-y-5">
            <main className="space-y-5">
              <section className="rounded-2xl border border-border bg-background p-4 shadow-sm">
                <div className="mb-3 flex items-center justify-between">
                  <h2 className="text-3xl font-bold text-foreground">Explore Stores</h2>
                  <Link href="/store?view=all">
              ...
              <section className="rounded-2xl border border-border bg-background p-4 shadow-sm">
                <div className="mb-3 flex items-center justify-between">
                  <h2 className="text-3xl font-bold text-foreground">Popular Stores</h2>
                  <Link href="/store?view=all">
                    <Button variant="outline" size="sm" className="rounded-lg">
                      View all <ChevronRight className="h-4 w-4" />
                    </Button>
              ...
              <section className="rounded-2xl border border-border bg-background p-4 shadow-sm">
                <div className="mb-4 flex items-center justify-between">
                  <h2 className="text-3xl font-bold text-foreground">Nearby Stores</h2>
                ...
                <div className="grid gap-4 md:grid-cols-[280px_minmax(0,1fr)]">
                  ...
                  <div className="space-y-3">
                    {allStores.slice(6, 9).map((store) => (
                      <div key={store.id} className="flex flex-col gap-3 rounded-xl border border-border p-3 md:flex-row md:items-center md:justify-between">
                        <div>
                          <h3 className="text-xl font-bold text-foreground">{store.name}</h3>
                          <p className="text-sm text-muted-foreground">
                            {store.distance} • <span className="font-semibold text-primary">Open</span> • {store.openHours}
                          </p>
                        </div>
