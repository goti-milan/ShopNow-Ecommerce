...
function CartContent() {
    ...
    return (
        <div className="min-h-screen bg-surface pb-20 pt-8 font-sans">
                        ...
                        <div className="lg:col-span-4 lg:sticky lg:top-24 h-fit space-y-6">
                            <div className="bg-background rounded-3xl border shadow-sm p-8 space-y-8">
                                <div className="flex items-center justify-between">
                                    ...
                                    >
                                        <span className="text-sm font-bold text-muted-foreground group-hover:text-primary transition-colors">Select All</span>
                                        <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${isAllSelected ? 'bg-primary border-primary' : 'border-border'}`}>
                                ...
                                <div className="bg-muted p-6 rounded-2xl border border-dashed border-border space-y-4">
                                    ...
                                    <div className="flex gap-2">
                                        <div className="relative flex-1">
                                            <Input
                                                ...
                                                className="rounded-xl border-border h-12 font-bold focus-visible:ring-primary/30"
                                            />
                                            {appliedDiscount > 0 && (
                                                <div className="absolute right-3 top-1/2 -translate-y-1/2">
                                                    <div className="w-5 h-5 rounded-full bg-accent0 flex items-center justify-center">
                                    ...
                                    {couponMessage && (
                                        <p className={`text-xs font-bold animate-in fade-in slide-in-from-top-1 ${couponMessage.includes("Invalid") ? "text-primary" : "text-primary"}`}>
                                            {couponMessage}
                                        </p>
                                    )}
                                        ...
                                        <>
                                            {selectedServiceItems.length > 0 && (
                                                <div className="flex justify-between items-center text-muted-foreground font-bold">
                                                    ...
                                                    <span className="text-foreground">₹{serviceSubtotal.toLocaleString()}</span>
                                                </div>
                                            )}
                                            {selectedProductItems.length > 0 && (
                                                <div className="flex justify-between items-center text-muted-foreground font-bold">
                                                    <span>{selectedProductItems.length} selected product{selectedProductItems.length !== 1 ? 's' : ''}</span>
                                                    <span className="text-foreground">₹{productSubtotal.toLocaleString()}</span>
                                                </div>
                                            )}
                                        </>
                                    ) : (
                                ...
                                <div className="py-6 border-y border-border space-y-4">
                                    {appliedDiscount > 0 && (
                                        <div className="flex justify-between items-center text-primary font-bold">
                                            <div className="flex items-center gap-2">
                                                <Badge variant="outline" className="border-primary text-primary bg-accent text-[10px] h-5">COUPON APPLIED</Badge>
                                                <span className="text-sm">Discount</span>
                                            </div>
                                            <span className="text-primary">- ₹{appliedDiscount.toLocaleString()}</span>
                                        </div>
                                    )}
                ...
                <div className="mt-20 space-y-8">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                        {PRODUCTS.slice(0, 5).map((product) => (
                            <div key={product.id} className="bg-background p-4 rounded-2xl border border-border shadow-sm hover:shadow-xl transition-all group">
                                <div className="relative aspect-square bg-muted rounded-xl overflow-hidden mb-4 p-4 border border-border">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="pt-1 opacity-0 group-hover:opacity-100 transition-opacity translate-y-2 group-hover:translate-y-0 duration-300">
                                        <Button
                                            ...
                                            className="w-full bg-primary hover:bg-primary-dark text-white !text-[10px] h-7 px-3 font-bold rounded-lg shadow-sm shadow-orange-100 whitespace-nowrap"
                                        >
                                            Add to Cart
                                        </Button>
                                    </div>
...
function ProductSection({ items, selectedIds, onSelect, onSelectAll, onRemove, onUpdateQty }: ProductSectionProps) {
    ...
    return (
        <div className="space-y-6">
            <div className="bg-background p-6 rounded-2xl border shadow-sm flex items-center justify-between">
...
function ServiceSection({ items, selectedIds, onSelect, onSelectAll, onRemove, onReschedule }: ServiceSectionProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="space-y-6">
                {items.map((item) => (
                    <div key={item.id} className="bg-background p-6 rounded-2xl border shadow-sm hover:shadow-lg transition-all relative group">
                        <div className="flex items-start gap-5">
                            <div
                                onClick={() => onSelect(item.id)}
                                className={`w-5 h-5 mt-1 rounded border flex items-center justify-center cursor-pointer transition-colors shrink-0 ${selectedIds.has(item.id) ? 'bg-primary border-primary' : 'border-border'}`}
                            >
                                {selectedIds.has(item.id) && <Check className="w-3.5 h-3.5 text-white" strokeWidth={3} />}
                            </div>

                            <div className="relative w-28 h-28 bg-muted rounded-2xl overflow-hidden shrink-0 border border-border">
                                    ...
                                    <div>
                                        ...
                                        {item.provider && (
                                            <div className="flex items-center gap-2 mt-1">
                                                ...
                                                <span className="text-sm text-muted-foreground font-medium">{item.provider}</span>
                                                <span className="flex items-center gap-1 text-xs text-primary font-bold"><ShieldCheck className="w-3 h-3" /> Verified</span>
                                            </div>
                                        )}
                                    </div>
...
function RescheduleModal({
    ...
    return (
        <div
            className="fixed inset-0 z-50 bg-secondary/50 backdrop-blur-sm flex items-center justify-center p-4"
        ...
        >
            <div className="bg-background w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="p-6 space-y-5">
                    <div className="flex items-center gap-4 px-4 py-3 bg-accent border border-primary/20 rounded-2xl text-sm">
                        <CalendarDays className="w-5 h-5 text-primary shrink-0" />
                        <div>
                            <p className="text-xs text-muted-foreground font-medium">Current booking</p>
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-foreground uppercase tracking-wide">New Date</label>
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-3 rounded-xl border border-border text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
                            />
                        </div>
                    </div>

                    <div className="space-y-2">
                        <label className="text-xs font-bold text-foreground uppercase tracking-wide">New Time Slot</label>
                    ...
                    <div className="flex gap-3 pt-1">
                        ...
                        <Button
                            className="flex-1 bg-primary hover:bg-primary-dark text-white rounded-xl font-bold shadow-lg shadow-orange-100"
                            disabled={!date || !time}
